...
exports.create = async (req, res, next) => {
    const neighborhoodName = (stripTags(req.body.neighborhoodName || '')).trim();
    const phoneCode = (stripTags(req.body.phoneCode || '')).trim();
    const active = (parseForm(req.body.active)).toString();
    const request = { neighborhoodName, phoneCode, active };

    try {
        return await Neighborhood.create(request).then(() => {
...
exports.findAll = async (req, res, next, code = 200) => {
    ...
    const { where } = getCondition(req.query, 'neighborhoodName');
    const { order } = getOrder(req.query.order, (req.query.orderName || 'neighborhoodName'));

    await Neighborhood.findAndCountAll({
        where,
        limit,
        offset,
        order,
...
exports.findOne = async (req, res) => {
    ...
    await Neighborhood.findByPk(id, { attributes: { exclude: ['id', 'createdAt'] } }).then((data) => {
        if (data) {
...
exports.update = async (req, res, next) => {
    ...
    data.neighborhoodName = data.neighborhoodName === '' ? delete data.neighborhoodName
        : (stripTags(req.body.neighborhoodName || '')).trim();
    data.phoneCode = data.phoneCode === '' ? delete data.phoneCode : (stripTags(req.body.phoneCode || '')).trim();

    await Neighborhood.update(data, {
        where: { uuid },
    }).then((num) => {
...
exports.updateAll = async (req, res, next) => {
    ...
    await Neighborhood.update({ active: '0' }, {
        where: { uuid },
    }).then((num) => {
...
exports.deleteAll = async (req, res, next) => {
    try {
        return await Neighborhood.destroy({
            where: {},
...
exports.child = async (req, res) => {
    ...
    const { offset, limit, page } = paginations(req, 'neighborhoodName');
    const { where } = getCondition(req, 'neighborhoodName');
    const { order } = getOrder(req, 'neighborhoodName');

    await Neighborhood.findAll({
        where: { uuid },
        attributes: {
        ...
        include: {
            ...
            offset,
            order,
            attributes: [[Sequelize.literal(' count(*) over ()'), 'count'], 'uuid', 'neighborhoodName', 'active', 'updatedAt'],
        },
    }).then((data) => {
        if (data) {
            return res.status(200).setHeader('Content-Type', 'application/json').json({
        ...
        return res.status(401).setHeader('Content-Type', 'application/json').json({
            ...
            code: 401,
            data: [],
            message: 'Cannot find neighborhood id.',
        });
    }).catch((err) => {
        return res.status(503).setHeader('Content-Type', 'application/json').json({
            status: false,
