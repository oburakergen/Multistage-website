...
exports.create = async (req, res, next) => {
    const districtName = (stripTags(req.body.districtName || '')).trim();
    const phoneCode = (stripTags(req.body.phoneCode || '')).trim();
    const active = (parseForm(req.body.active)).toString();
    const request = { districtName, phoneCode, active };

    try {
        return await District.create(request).then(() => {
...
exports.findAll = async (req, res, next, code = 200) => {
    ...
    const { where } = getCondition(req.query, 'districtName');
    const { order } = getOrder(req.query.order, (req.query.orderName || 'districtName'));
...
exports.findOne = async (req, res) => {
    ...
    await District.findByPk(id, { attributes: { exclude: ['id', 'createdAt'] } }).then((data) => {
        if (data) {
...
exports.update = async (req, res, next) => {
    ...
    data.districtName = data.districtName === '' ? delete data.districtName : (stripTags(req.body.districtName || '')).trim();
...
exports.updateAll = async (req, res, next) => {
    ...
    await District.update({ active: '0' }, {
        where: { uuid },
    }).then((num) => {
...
exports.child = async (req, res) => {
    ...
    const { offset, limit, page } = paginations(req, 'districtName');
    const { where } = getCondition(req, 'districtName');
    const { order } = getOrder(req, 'districtName');
    ...
    await District.findAll({
        ...
        include: {
            ...
            offset,
            order,
            attributes: [[Sequelize.literal(' count(*) over ()'), 'count'], 'uuid', 'districtName', 'active', 'updatedAt'],
        },
    }).then((data) => {
        if (data) {
            return res.status(200).setHeader('Content-Type', 'application/json').json({
        ...
        return res.status(401).setHeader('Content-Type', 'application/json').json({
            ...
            code: 401,
            data: [],
            message: 'Cannot find District id.',
        });
    }).catch((err) => {
        return res.status(503).setHeader('Content-Type', 'application/json').json({
            status: false,
            code: 503,
            data: [],
            message: err.message || 'Error retrieving District with id=.',
        });
    });
};
